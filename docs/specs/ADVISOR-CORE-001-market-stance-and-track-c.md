...
### 7-h. F3 — 이평 구조 축 (구현 완료 2026-08-13)

M1 판세에 **개별 종목 축이 없어** *"삼성전자가 20일선 하단에서 변곡하는 장대양봉"* 같은 걸
못 봤다. 시장·섹터 레벨만 봐서 사용자 체감이 가장 큰 갭이었다.

**사용자 이평 체계 확정** (`docs/a_wanted/user_want_spec.md` 의 "인터뷰 대기" 해소):

> 일봉 7·13·20·60·120 / 주봉 5·10·20 / 월봉 **종가 기준 7월선 지지력**.
> 단기–중기–장기 이평 간 **변곡 탐지** + 이격도의 **미적분적 포착**(1차 수렴/발산 · 2차 가속/감속).
> **주도 종목뿐 아니라 지수 차트에도 해당하는 구조적 차트해석법.**

| 항목 | 내용 |
|---|---|
| 신규 모듈 | `core/signal/ma_structure.py` — 순수(LLM 0·I/O 0), **지수·종목 공용 렌즈** |
| 신규 수집 | **0** — `chart_ohlcv`·`universe_membership` read (가드 #11) |
| 배선 | `StructureRead`/`StructureDigest` → `StanceFacts.structures` → `facts_json` 자동 영속 |
| 렌더 | 지수 2 + 거래대금 상위 5종, 지수 블록 **바로 뒤**(예산 절단은 아래에서부터) |
| 예산 | `RENDER_BUDGET_CHARS` 2,000 → **3,000** (늘린 몫은 전부 *사실* — §3-c 역전 방향과 정합) |
| 가드 | 차트가 `as_of` 보다 낡으면 **구조를 만들지 않는다**. 빠진 종목 수를 md 에 숫자로 남기고, 전멸 시 "산출 0 — 이평·변곡을 언급하지 말 것" 경고 |

**핵심 설계**: 이격도의 *현재값*만 보면 정지 사진이라 국면 전환을 못 본다. 1차 미분(벌어짐/수렴)과
2차 미분(가속/감속)을 함께 산출해 *"20일선 아래지만 빠르게 좁혀지며 가속"* = 돌파 시도를
**코드가 먼저 감지해 이름 붙인다**(F2 와 같은 결). 판단·해석은 LLM 몫.

`collectors/charts.py::compute_indicators` 는 일봉 13 · 주봉 5 를 **가산**(기존 키 보존 → 소비처 무영향).
`collectors/anchors.py::resample_ohlcv` public 승격(주/월 resample 규약 단일화).
