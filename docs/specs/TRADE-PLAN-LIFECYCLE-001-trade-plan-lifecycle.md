...
- **2단계 — 매수대기 *단계* (대기 진입가 + 승격 사유 + 라벨)** ⭐ 다음: 단순 "조건부 진입가 숫자"가 아니라 **매수대기 단계로 격상**. (a) `alpha_posture.conditional_entry` 채우기(눌림 분할 / 돌파 / 추세 하단 **방법 선택** + 가격) — BRAIN-ALPHA-FLEXIBILITY 조건부진입 이관, compute_scorecard 에 price/ma 실어 산출. (b) **단계 라벨 + 승격 사유** 파생(wait+품질근접+conditional_entry → "매수대기") + (c) **진입 시나리오 시계열**(트리거·조건). <!-- SPEC:INTERVIEW-SLOT name="watching-tier" 승격 임계(점수 근접 기준)·진입 방법 선택 규칙(과열→눌림·박스→돌파·정석→추세하단)·시나리오 깊이 -->
