...
**이번 세션에 굳힌 판단 (2026-06-19 — 노출·알림 단 종목코드 제거)**:
- **노출/알림 단 = 종목명 / LLM 입력 = 코드 병기 OK (경계선 확정)**: 사용자가 보는 모든 화면·텔레그램은 종목명만(코드는 "내가 봐도 모른다"). 반대로 분석가 sub-task 프롬프트·시황 md·전략가 지시문은 LLM 입력이라 코드 병기가 오히려 정확도에 유리 → 안 건드림. "노출단/알림단"의 정확한 범위 = 사람이 읽는 출력만.
- **종목명 해석은 중앙 리졸버 1개로 (땜질 금지)**: 같은 `이름(코드)`/코드폴백 패턴이 6+곳에 흩어져 있었음. 곳곳을 고치는 대신 `resolve_stock_name(ticker, hint)` 단일 진입점(hint→멤버십 DB→정적 매핑→최후 코드)을 만들고 전부 통과시킴. 데이터 타입(`Holding`/`ClosedFill`/`FillEntry`)이 애초에 종목명을 안 실어 webapp이 코드를 쓸 수밖에 없던 구조 결함이 진짜 근본 원인 — 뷰 레이어에서 `display_name` 주입으로 해소.
...
## 🎯 다음에 할 일 (Top 3) — 관심종목 페이지 후속 + 두뇌 잔여

**🏷 노출·알림 단 종목코드 → 종목명 완료 (2026-06-19)** — `resolve_stock_name` 중앙 리졸버로 텔레그램·데스크·계좌·브리핑 전부 종목명화(1344 passed). (잔여 minor 백로그: 정적 매핑·멤버십 어디에도 없는 종목은 여전히 코드 폴백 — 실사용에서 사례 보이면 정적 매핑 추가 or 멤버십 백필 점검.)

### 1. 종목 상세 페이지 + 채팅 이어가기 ⭐
- **왜**: 인라인 시나리오 + `/chat?ticker=` 링크까지만 됨. 종목별 근거 자료 상세 + 거기서 채팅 prefill 수신이 사용자 명시 요구.
- **범위**: `webapp/src/app/watchlist/[ticker]/page.tsx` 신규(플랜 시계열+근거 자료) + `/chat` 가 `?ticker=` 받아 prefill. 단일 종목 플랜 조회 API 1개(기존 read 조립).
- **예상 산출**: 관심종목 클릭 → 상세 + "이 종목 채팅으로" 자연 연결.

### 2. 데스크 "지켜보는 권고" 섹션 개선 (메모리 task)
- **왜**: 관심종목 페이지가 자리잡았으니, 데스크는 관심 덤프 빼고 actionable(매수대기+진입)만. [[project_watchlist_management]] follow-up.
- **범위**: `core/account/desk_view.active_recommendations_view` interest 제외(또는 매수대기+진입만) + 데스크 섹션 워딩 정리. 관심 전체는 /watchlist 로 안내.
- **예상 산출**: 데스크 = actionable 권고만, 관심 풀은 /watchlist.

### 3. 데이터 충실화 + 두뇌 잔여(M3b)
- **왜**: chart_ohlcv 캐시 없는 중소형 → 컨셉/거래량 "—"(미분류 16). + BRAIN-ALPHA-FLEXIBILITY M3b(sector_rs·wave→약세장 bear_override) 미완.
- **범위**: 장후 `refresh_all_tickers` 누적이 미분류 해소하는지 + 자율 cron 첫 실 발화 검증 / M3b = `compute_scorecard` 에 섹터RS·파동 LLM 입력.
- **예상 산출**: 거래량/컨셉 빈칸 채워짐 + 약세장 차등 진입 동작.

(이월: **전략가 추론 감사 뷰** — 3줄 압축이 깊이를 가림. 권고 클릭 → full reasons·기간·regime·명제 펼침 + 상태 필터·정렬. team_outputs read 조립(가드 #11).)
