...
**이번 세션에 굳힌 판단 (2026-06-20 — 백테스트 + 알파 철학 + 후보층 강화)**:
...
**직전 세션 판단 (2026-06-16 — 다단 메뉴 B-MS1 + 다층 단계)**:
- **B→C 애자일 합본 (결정론 "선택 머신"은 안 만든다)**: 사용자 "끝없는데 의미 있나"에 동의. 결정론은 *객관 가격대 계산*만(스윙·MA·ATR·52w = 산수, 신뢰도 질문 자체 없음) 해서 LLM에 **사실로 주입**(숫자 환각 차단 — prism·cited_scores 누수 교훈). 상황별 선택 룰 트리는 LLM+진화. 소수 절대 가드레일(오닐 −7%·종가)만 결정론 강제(코드 clamp + menu_bound 감사). 프리즘급 다단 시나리오는 그대로 나오되 "환각=숫자 출처 결정론 / 잦은 변경=deviation 로그+측정(C)"로 막힘.
- **라이브가 메뉴 버그를 잡는다 (단위테스트로 안 보임)**: 000660 라이브에서 stale 스윙저점(2.36M인데 808K)·동일가 중복(스윙고점==52주고가)이 드러남. 합성 데이터 단위테스트는 통과해도 실 OHLCV라야 보이는 품질 결함 — 라이브 1회가 필수.
- **다층 단계 = 파생 라벨, 새 판단 아님**: 관심→매수대기→buy를 *이미 있는 판단(점수·verdict·alpha_posture·메뉴)을 조립·표면화한 라벨*로. "재료는 다 있고 연결만 없다." wait을 죽은 통이 아니라 사유·시나리오 가진 단계로 = legibility=쓰는 데스크 차별화. 단계=상태지 강제순서 아님(현시점 부합 단계 진입).
- **신고가권 목표 = 결정론으로 못 만든다(현대차 사례)**: 리레이팅 100만원은 과거에 없던 가격 → 차트 계산기가 만들 수 없음(환각 방지 대가). 미래 전망 목표는 measured-move+LLM forward(3단계). 신고가권은 "목표 열림/trailing"으로.

**직전 세션 판단 (2026-06-15 6세션 — 트레이드 플랜 설계)**:
- **결정론 = 계산기, 결정자 아님 (이 프로젝트 가격 로직의 헌법)**: 손절·목표·진입·분할 모든 레벨에서 결정론은 *후보 메뉴·신호*만 계산(스윙저점·ATR·MA·이평 꺾임 = 참/거짓), *어느 게 이 상황에 맞나(적재적소)+임계+수정*은 판단(LLM·룰). "결정론이 세상 모든 사례를 타이트하게 맞춰야 하나"=함정·영원히 아니오. 손절도 단일 공식 아님(후보 多·객관적), 목표는 후보 少·수정 잦음. = "가드레일 있는 C"의 일반화.
- **트레이드 플랜 = 스냅샷 아니라 살아있는 시계열 객체**: (종목×트랙)당 대기→진입→보유→청산 상태 + 다단 레벨이 매 cadence 진화(목표 동적 수정·사유 로그)+레벨 도달 알림. 목표가 도달 = 자동매도 아니라 마일스톤(regime 조건부 trailing/매도, prism 차용).
- **큰 비전은 단계로 쪼개 한 번에 안 한다**: 5단계(쉬운·결정론 레벨 → 어려운·동적 목표·진화·알림). 사용자 인지 부하 신호("다 깊게 파?")에 즉시 한 발 빼고 단계화·추천 제시한 게 유효.

**직전 세션 판단 (2026-06-15 5세션 — 라이브 검증 + 위험 게이트)**:
- **"전부 wait" 진범은 연쇄였다 (regime→점수하한→분산일 kill)**: 라이브로 한 겹씩 벗기니 진짜 blanket 은 **분산일 kill-switch(dd≥4, 시장전체)**. regime 만 고친 건 절반이었음. 교훈 = 단위 테스트로 "후보=buy" 증명해도 **라이브 funnel 끝까지 돌려야 진짜 게이트가 보인다**.
- **복합 위험 게이트 = 폭락 방어 ∧ 알파 유연 양립**: 느린 신호(dd) 단독 blanket 은 "완만 분산에 전부 wait + 당일 급락은 못 잡는" 최악. 빠른 신호(당일 급락·breadth 붕괴·VIX)+느린 신호(지속 분산 dd≥6) 결합 → 진짜 위험일만 blanket, 평상시 차등. 사용자 우려(반대매매·프로그램 매도일 회피) 직접 반영.
...
**마지막 작업일**: 2026-06-16 2세션 (**관심종목 종목관리 페이지 + 큐레이션 + 트레이드플랜 2단계 — 대형 세션**). 직전 = 2026-06-16 B-MS1.
**마지막 세션 로그**: [2026-06-16_watchlist-management-curation-2.md](c_worked/2026-06-16_watchlist-management-curation-2.md). 직전 = [2026-06-16_trade-plan-menu-b-ms1-and-tier-funnel.md](c_worked/2026-06-16_trade-plan-menu-b-ms1-and-tier-funnel.md).
**산출**: 신규 `collectors/{universe_membership,volume_bull,universe_curation}.py` + `core/watchlist_view.py` + `server/api/watchlist.py` + `webapp/.../watchlist/*`(페이지·보드) + `universe_membership` 테이블(v17~v19) + 트레이드플랜 2단계(`alpha_posture` enrich/funnel_stage) + 데스크 리뉴얼·dedup/round-trip 버그 수정. 전체 1341 passed·build tsc 0.
**Git**: 세션 코드 미커밋 → wrap-up 이 feat(코드+신규모듈)+docs(wrap-up) 커밋 + main + push.

---

## 🎯 다음에 할 일 (Top 3) — 백테스트로 검증하며 결정론 후보층 강화

**🔬 백테스트 1차 = Track A 본진 검증 (2026-06-20)** — 이평정배열+상대강도 분기 보유가 시장 +8.7%p·낮은 MDD·하락장 2배(단 생존편향 상한선). 상세 = `docs/BACKTEST-FINDINGS-AND-ALPHA-ROADMAP.md`. 결정: **결정론 후보층을 먼저·타이트하게 강화**(측정 가능), LLM 다듬기는 forward 루프 후.

### 1. 결정론 후보층 강화 — 이평선/차트모양 인터뷰 먼저 ⭐ (내일)
- **왜**: edge는 결정론에 있고(측정 가능), LLM은 hindsight로 백테스트 불가. 후보 질이 시스템 천장. IC가 최상위에서만 +라 *좁고 확신 높은 슬라이스*를 떠먹여야.
- **방식**: ① 사용자가 월/주/일봉별 **중요 이평선 조건 + 차트 모양(이미지 포함)** 던짐(인터뷰 먼저) → ② Track A/B **타임프레임 정합 지표 + confluence 조건(정배열 AND 상대강도 AND 신고가 초기 AND 이평지지) + 가드(장세·가속초기·유동성·손절)** 를 결정론 후보 생성에 구현 → ③ 조건마다 **백테스트 A/B**로 타이트하게. 게이트 아닌 *메뉴*(LLM 결정).
- **예상 산출**: 고확신 후보 슬라이스 + edge 상승 측정. (BRAIN-QUALITY / WAVE-ALPHA 재료)

### 2. 정직한 백테스트 인프라 (BACKTEST SPEC)
- **왜**: +8.7%p가 생존편향·비용 제거 후에도 진짜인지 = 모든 결정론 강화의 검증 도구.
- **범위**: 상폐 포함 point-in-time 유니버스(생존편향 제거) + 실비용·슬리피지 + 실 KOSPI 벤치마크. 현 R&D 프로브(`scripts/_backtest_*.py`)를 정식 SPEC 엔진으로 격상.
- **예상 산출**: 신뢰할 수 있는 edge 측정치 + 재사용 백테스트 엔진.

### 3. 무인 forward "진실의 루프"
- **왜**: LLM 가치·실현 수익은 forward로만 측정 가능. track record 0 해소의 본체.
- **범위**: 24/7 무인 가동(OPS/절전 방어) + 매일 벤치마크 대비 정직 채점 + 회고/진화팀(EVOLUTION 미작성)이 결과로 두뇌 교정.
- **예상 산출**: 진짜 track record 누적 + 자기진화 루프 한 바퀴.
