...
**이번 세션에 굳힌 판단 (2026-06-15 3세션 — 두뇌 알파 유연성)**:
...
**마지막 작업일**: 2026-06-15 3세션 (**두뇌 알파 유연성 SPEC + M1 결정론 차등 변조**). 직전 = 2026-06-15 텔레그램 절전 사고.
**마지막 세션 로그**: [2026-06-15_brain-alpha-flexibility-spec-m1.md](c_worked/2026-06-15_brain-alpha-flexibility-spec-m1.md). 직전 = [2026-06-15_telegram-sleep-incident-misfire-tolerance.md](c_worked/2026-06-15_telegram-sleep-incident-misfire-tolerance.md).
...
## 🎯 다음에 할 일 (Top 3) — 두뇌 알파 유연성 (BRAIN-ALPHA-FLEXIBILITY M2~M6)

**🧠 SPEC 박고 M1(결정론 차등 변조) 완료 → LLM 절반(M2)부터 (2026-06-15 3세션 후)** — `BRAIN-ALPHA-FLEXIBILITY-001` implementing. M1 `alpha_posture.py`(순수 함수)가 regime baseline × 섹터RS × 주도주 × 파동 × 과열도 → verdict 후보+조건부 진입 의도+선정 사유를 산출(17 테스트). 남은 건 이 후보를 **funnel·persona·watchlist 에 배선**해 라이브서 "전부 wait" 탈피를 증명하는 것. AUTO-SIGNAL 자율 cron 첫 발화=내일 18:05.

### 1. M2 — persona doctrine 전환 (가드레일 있는 C 의 LLM 절반)
- **왜**: M1 은 결정론 후보만 만든다. 전략가가 그걸 받아 "반박할 사실 있나?" 검증자로 동작해야 blanket 보수 강등이 막힌다.
- **범위**: `agents/strategists/track_a/persona.md`·`track_b/persona.md` 의 Reasoning Doctrine 을 **범주 게이트 폐기 + AlphaPosture 후보 소비자 + deviation 사실근거(`llm_deviation_reason`) 로그** 로 재집필. Track B kill-switch 는 유지하되 차등 변조 적용.
- **예상 산출**: regime 에 통째 안 눌리는 전략가 판단.

### 2. M3 — funnel 주입 + 라이브 검증
- **왜**: alpha_posture 가 funnel 에 연결돼야 실제 권고에 반영된다. 오늘 "strong_bull 전부 wait" 의 직접 탈피 확인 지점.
- **범위**: `core/signal/auto_signal.py` — compute_scorecard 에 **섹터RS·파동(α→bool, `collectors/anchors.py`)** 추가 + `build_prefetched_entries`/directive 에 AlphaPosture 후보 주입 + conditional_entry 를 가격으로 채움(entry→stop 보간, RB-MS2 패턴). 라이브 1회: buy 후보 ≥1 발생 확인.
- **예상 산출**: 매일 매수/관망이 종목별로 갈리는 권고 스트림.

### 3. M4 watchlist 선정 강화 + M5 웹 더블체크
- **왜**: 선정 자체가 거래대금+RS만이라 비주류·무파동 섞임 / buy 후보 catalyst 진위 미검증.
- **범위**: `collectors/screening.py rank_candidates` 가중에 파동·주도주·섹터RS 편입 + config + **미장 watchlist source** 결정 / buy 후보 Gemini search grounding(신규 capability·비용 가드) + `core/strategist/recommendation.py` data_json 가산(verdict_candidate·selection_reason·web_check).
...
(✅ 해소: **BRAIN-ALPHA-FLEXIBILITY-001 SPEC + M1 결정론 차등 변조** (두뇌 알파 유연성 = BRAIN-QUALITY 첫 자식, M2~M6 잔여). / 직전: 자동 권고 funnel M1~M6 (BODY-AUTOMATION) / Phase 2 4기둥 roadmap.)
