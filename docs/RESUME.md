...
**이번 세션에 굳힌 판단 (2026-06-15 4세션 — M2 doctrine + M3a 배선)**:
...
**마지막 작업일**: 2026-06-15 4세션 (**M2 persona doctrine 전환 + M3a funnel 결정론 배선**). 직전 = 2026-06-15 SPEC+M1.
**마지막 세션 로그**: [2026-06-15_brain-alpha-flexibility-m2-m3a.md](c_worked/2026-06-15_brain-alpha-flexibility-m2-m3a.md). 직전 = [2026-06-15_brain-alpha-flexibility-spec-m1.md](c_worked/2026-06-15_brain-alpha-flexibility-spec-m1.md).
...
## 🎯 다음에 할 일 (Top 3) — 두뇌 알파 유연성 (BRAIN-ALPHA-FLEXIBILITY 라이브·M3b·M4/5)

**🧠 M2(doctrine)+M3a(결정론 배선) 완료 → 라이브 증명 + LLM 입력(M3b) (2026-06-15 4세션 후)** — `BRAIN-ALPHA-FLEXIBILITY-001` implementing. 결정론 후보가 전략가 프롬프트 주입 + 권고 영속까지 배선됨(92 테스트·전체 1247). 결정론상 strong_bull→buy 후보 보장(단위 증명). 남은 건 ① 실 Gemini 로 라이브 탈피 실증 ② sector_rs·wave LLM 입력으로 약세장 bear_override 활성. AUTO-SIGNAL 자율 cron 18:05.

### 1. 라이브 검증 — strong_bull buy 후보 실증
- **왜**: 결정론은 단위 증명됐으나 실 Gemini 가 후보를 채택해 buy 를 발행하는지 = "전부 wait" 탈피의 최종 확인.
- **범위**: 실 Gemini 1회 — `run_auto_signal_job("intraday1")` 또는 probe(서버/스냅샷 필요). strong_bull 종목 buy 후보 ≥1 + `rec.data.alpha_posture` 영속 + `llm_deviation_reason` 동작 확인. 차트 DB-first 병목 주의.
- **예상 산출**: 라이브 권고에 매수/관망 종목별 분기 실증.

### 2. M3b — sector_rs·wave LLM 입력 배선
- **왜**: 현재 sector_rs·wave=None 이라 약세장 bear_override(강세섹터+주도주+파동+눌림목=눌림목 타점)가 안 뜸. bullish/neutral 차등만 동작.
- **범위**: `core/signal/auto_signal.py compute_scorecard` 에 종목 섹터 RS(`classify_theme`→`get_theme_sector_mapping`→sector_rs max)·파동 생존(`compute_alpha_3tf` weekly/daily label=="sweet"→bool) 추가. LLM Haiku·30일 캐시·graceful(실패 시 None).
- **예상 산출**: 약세장에서도 주도주 눌림목 매수 후보.

### 3. M4 watchlist 선정 강화 + M5 웹 더블체크
- **왜**: 선정 자체가 거래대금+RS만이라 비주류·무파동 섞임 / buy 후보 catalyst 진위 미검증.
- **범위**: `collectors/screening.py rank_candidates` 가중에 파동·주도주·섹터RS 편입 + config + **미장 watchlist source** 결정 / buy 후보 Gemini search grounding(신규 capability·비용 가드) + `recommendation.py` data_json 가산(web_check).
...
(✅ 해소: **BRAIN-ALPHA-FLEXIBILITY-001 M2 doctrine + M3a 결정론 배선** (후보 주입·영속 라이브 경로 연결, 라이브 검증·M3b·M4/5 잔여). / 직전: SPEC+M1 결정론 차등 변조 / 자동 권고 funnel M1~M6.)
