...
**현재 위치**: morning_pre 실전 Gemini 호출 성공 + 메시지 포맷 정제 완료.
브리핑 온디맨드 플랫폼(BRIEFING-ON-DEMAND-001) SPEC 작성까지 끝남 — 첫 SPEC.
다음: SPEC 구현(텔레그램 봇 + 공통 API + DB v3) 또는 knowledge/canon 주입 중 선택.
...
- API: `/api/briefings/*`, `/api/positions/*`, `/api/pipelines/*`
...
### 미완 또는 의도적 공백
- `knowledge/canon/*.md` 의 TODO (사용자 주입 대기)
...
- `scripts/demo.py`, `tests/test_e2e.py`, `server/api/demo.py` 의 `teams.orchestrator` 잔재
- `docs/STRUCTURE.md` 구버전 (teams/ 기준, 실제는 pipelines/)
- 09:30 / 13:00 / 16:00 / 19:00 파이프라인 미착수 — BRIEFING-ON-DEMAND v2 로드맵과 짝
- **워킹트리 전부 미커밋** (Phase 2 + 오늘 변경). 다음 세션 시작 시 정리 겸 묶음 커밋 권장.
...
- **워크트리 위치 주의**: 현재 작업은 `.claude/worktrees/brave-hertz-b82ac4` 에서만 진행 — 메인 레포엔 미반영. 커밋 후 `git merge claude/brave-hertz-b82ac4` 필요.
