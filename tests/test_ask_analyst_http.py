"""INFRA-RUNTIME-EFFICIENCY-001 (a) — scripts.ask_analyst 가 in-process
run_analyst 가 아니라 server HTTP endpoint 를 호출하는지 검증.

핵심 본질: CLI 매 실행 = 새 Python 프로세스 = BGE-m3 재로딩. server 경유로 바꿔
프로세스 1 개 안에서 lru_cache reuse 가 가능해진다. 본 테스트는 httpx
AsyncClient 를 mock 으로 가로채 정상·연결실패·404·500 응답 시 exit code
와 stdout/stderr 가 의도대로인지 확인.
"""
...
ask_mod = importlib.import_module("scripts.ask_analyst")
...
def test_success_prints_text_and_metadata(tmp_path, monkeypatch, capsys):
    ...
    with patch("httpx.AsyncClient", _factory):
        rc = _run(ask_mod._ask("principle_guardian", "정량 룰 검증"))

    assert rc == 0
    assert captured_client["c"].calls[0][0] == (
        "http://test-server:9999/api/analysts/principle_guardian/chat"
    )
    out = capsys.readouterr().out
    assert "분석 결과 본문" in out
    assert "1,234 chars" in out
    assert "RAG 2 chunks" in out
...
def test_connect_error_returns_exit_3_with_clear_message(tmp_path, monkeypatch, capsys):
    ...
    with patch("httpx.AsyncClient", lambda *a, **kw: _MockClient(_conn_err)):
        rc = _run(ask_mod._ask("principle_guardian", "q"))
...
def test_404_returns_exit_2(tmp_path, monkeypatch, capsys):
    ...
    def _not_found(url, payload):
        return _MockResponse(404, {"detail": "analyst 'foo' 가 등록되지 않았습니다"})

    with patch("httpx.AsyncClient", lambda *a, **kw: _MockClient(_not_found)):
        rc = _run(ask_mod._ask("foo", "q"))
...
def test_provider_param_forwarded_to_server(tmp_path, monkeypatch):
    monkeypatch.setattr(ask_mod, "QUERIES_DIR", tmp_path / "queries")
    ...
    with patch("httpx.AsyncClient", _factory):
        _run(ask_mod._ask("trader", "q", provider="claude_code"))
...
def test_no_provider_when_unspecified(tmp_path, monkeypatch):
    monkeypatch.setattr(ask_mod, "QUERIES_DIR", tmp_path / "queries")
