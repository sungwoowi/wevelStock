...
def test_returns_none_when_empty() -> None:
    assert get_last_run_before("morning_pre", "2026-04-24 10:00:00") is None


def test_finds_run_strictly_before_cutoff() -> None:
    upsert_parts("morning_pre", "run_before", _one_part())
    _force_created_at("run_before", "2026-04-24 07:30:00")

    result = get_last_run_before("morning_pre", "2026-04-24 09:00:00")
...
def test_excludes_rows_at_or_after_cutoff() -> None:
    upsert_parts("morning_pre", "run_after", _one_part())
    _force_created_at("run_after", "2026-04-24 09:30:00")

    assert get_last_run_before("morning_pre", "2026-04-24 09:00:00") is None


def test_picks_latest_when_multiple_before_cutoff() -> None:
    upsert_parts("morning_pre", "run_old", _one_part())
    upsert_parts("morning_pre", "run_new", _one_part())
...
def test_filters_by_pipeline_id() -> None:
    upsert_parts("morning_pre", "mp_run", _one_part())
...
def test_since_iso_filters_out_older_runs() -> None:
    upsert_parts("morning_pre", "yesterday_run", _one_part())
    upsert_parts("morning_pre", "today_run", _one_part())
...
def test_since_iso_none_keeps_no_lower_bound() -> None:
    upsert_parts("morning_pre", "old", _one_part())
